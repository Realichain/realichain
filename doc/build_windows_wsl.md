# How to compile and execute Realichain code on Windows 10

There are two possibilities of compiling Realichain code on Windows 10.
 ...
 * [Xming](http://www.straightrunning.com/XmingNotes/) latest version, if you are planning to compile using [unix compiling instruction](https://github.com/Realichain/realichain/blob/master/doc/build-unix.md)
...
4. Make shared folder between two locations
    ...
    * From Explorer create a directory named:
            ...
            C:\Users\<username>\realichain
    * From Ubuntu bash window create a link to it:
  
            ln -s /mnt/c/Users/<username>/realichain realichain   
...
5. Clone git repository into newly created directory and go to the right branch
            ...
            git clone https://github.com/Realichain/realichain.git
            cd realichain
...
2. Dependencies won't compile in a directory shared with Windows. We need to copy everything to the space private to
    WSL. Do not use .. here as it will lead you into different directory
        ...
        cp -r depends ~/realichain-depends

3. Go to depends folder and build dependencies (you may wish to build only 32-bit or 64-bit version)

        cd ~/realichain-depends
        make HOST=i686-w64-mingw32 -j`nproc`
        make HOST=x86_64-w64-mingw32 -j`nproc`
        cd ~/realichain
    It takes a while. You need to do it only once unless you delete realichain-depends directory
...
6. Build 32-bit debug build (from the Realichain root directory)
    
        ./configure --prefix=$HOME/realichain-depends/i686-w64-mingw32
        make -j`nproc`

7. Build 64-bit debug build (from the Realichain root directory)
     
        ./configure --prefix=$HOME/realichain-depends/x86_64-w64-mingw32
        make -j`nproc`

8. After check the directory to run GUI with __realichain-qt.exe__
    
        C:\Users\<username>\realichain\src\qt
----
## Second method - Ubuntu + Xming
1. Use existing paper [build-unix](https://github.com/Realichain/realichain/blob/master/doc/build-unix.md) and
...
4. From Ubuntu bash window start realichain-qt:

        cd /mnt/c/Users/<username>/realichain/src/qt
        ./realichain-qt 
