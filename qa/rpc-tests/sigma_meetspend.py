...
def activate_sigma_spend(denoms, spendsize, exp_spends, exp_remints, realichaind):
    for denom in denoms:
        count, size = denom
        for i in range(count):
            realichaind.nodes[0].mint(size)
    realichaind.nodes[0].generate(6)

    myaddr = realichaind.nodes[0].listreceivedbyaddress(0, True)[0]['address']
    args = {myaddr: spendsize}
    txid = realichaind.nodes[0].spendmany("", args)
    realichaind.nodes[0].generate(2)

    # Should be checked spends
    spends = realichaind.nodes[0].listsigmaspends(0)
