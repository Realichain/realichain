...
#if defined(NDEBUG)
# error "Realichain cannot be compiled without assertions."
#endif
...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool fLimitFree,
                              ...
                              bool isCheckWalletTransaction, bool markRealichainSpendTransactionSerial)
{
    ...
    if (tx.IsSigmaSpend()) {
        if(markRealichainSpendTransactionSerial)
                        ...
                        bool isCheckWalletTransaction, bool markRealichainSpendTransactionSerial)
{
    ...
    bool res = AcceptToMemoryPoolWorker(pool, state, tx, fLimitFree, pfMissingInputs, nAcceptTime, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, coins_to_uncache, isCheckWalletTransaction, markRealichainSpendTransactionSerial);
    if (!res) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx, bool fLimitFree,
                        ...
                        bool isCheckWalletTransaction, bool markRealichainSpendTransactionSerial)
{
    return AcceptToMemoryPoolWithTime(pool, state, tx, fLimitFree, pfMissingInputs, GetTime(), plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, isCheckWalletTransaction, markRealichainSpendTransactionSerial);
}


bool AcceptToMemoryPool(CTxPoolAggregate& poolAggregate, CValidationState &state, const CTransactionRef &tx, bool fLimitFree,
                        bool* pfMissingInputs, std::list<CTransactionRef>* plTxnReplaced,
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee, bool isCheckWalletTransaction, bool markRealichainSpendTransactionSerial) {
    bool res = AcceptToMemoryPool(mempool, state, tx, fLimitFree, pfMissingInputs, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, isCheckWalletTransaction, markRealichainSpendTransactionSerial);
        ...
        BOOST_FOREACH(const CTxIn &txin, tx.vin) {
            if(!txin.scriptSig.IsSigmaSpend()) {
                return state.DoS(
                    100, false,
                    REJECT_MALFORMED,
                    "CheckSpendRealichainTransaction: can't mix zerocoin spend input with regular ones");
            }
...
void ThreadScriptCheck() {
    RenameThread("realichain-scriptch");
        ...
        for (const auto& it : block.vtx) {
            ...
            if (!tx.IsCoinBase()) {
                AcceptToMemoryPool(
                    ...
                    false /* markRealichainSpendTransactionSerial */
                );
            }
...
//    if (nHeight > nMNPIBlock + (nMNPIPeriod * 9)) ret += blockValue / 40; // 313520 - 50.0% - 2015-08-03
    ...
    CAmount ret = 15 * coin; //15 or 7.5 REALI
