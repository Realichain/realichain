...
const char * const BITCOIN_CONF_FILENAME = "realichain.conf";
const char * const BITCOIN_PID_FILENAME = "realichaind.pid";
...
bool fNoDebug = false; //A temporary fix for https://github.com/Realichain/realichain/issues/1011
...
{
    //A temporary fix for https://github.com/Realichain/realichain/issues/1011
...
boost::filesystem::path GetDefaultDataDirForCoinName(const std::string &coinName)
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\realichain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\realichain
    // Mac: ~/Library/Application Support/realichain
    // Unix: ~/.realichain
#ifdef WIN32
    // Windows
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    fs::path realichainDefaultDir = GetDefaultDataDirForCoinName("realichain");
    if (!fs::is_directory(realichainDefaultDir)) {
        // try "zcoin" in case we're upgrading from pre-realichain version
        fs::path zcoinDefaultDir = GetDefaultDataDirForCoinName("zcoin");
        if (fs::is_directory(zcoinDefaultDir))
            return zcoinDefaultDir;
    }

    return realichainDefaultDir;
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No realichain.conf file is OK
...
bool RenameDirectoriesFromZcoinToRealichain()
{
    namespace fs = boost::filesystem;

    fs::path zcoinPath = GetDefaultDataDirForCoinName("zcoin");
    fs::path realichainPath = GetDefaultDataDirForCoinName("realichain");

    // rename is possible only if zcoin directory exists and realichain doesn't
    if (fs::exists(realichainPath) || !fs::is_directory(zcoinPath))
        return false;

    fs::path zcoinConfFileName = zcoinPath / "zcoin.conf";
    fs::path realichainConfFileName = zcoinPath / "realichain.conf";
    if (fs::exists(realichainConfFileName))
        return false;

    try {
        if (fs::is_regular_file(zcoinConfFileName))
            fs::rename(zcoinConfFileName, realichainConfFileName);

        try {
            fs::rename(zcoinPath, realichainPath);
        }
        catch (const fs::filesystem_error &) {
            // rename config file back
            fs::rename(realichainConfFileName, zcoinConfFileName);
            throw;
        }
    }
    catch (const fs::filesystem_error &) {
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Realichain Core copyright is not removed by accident
    if (copyright_devs.find(_("Realichain Core")) == std::string::npos) {
        strCopyrightHolders += '\n' + strPrefix + "The Realichain Core developers";
    }
