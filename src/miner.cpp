...
void BlockAssembler::FillFoundersReward(CMutableTransaction &coinbaseTx, bool fMTP, bool fShorterBlockDistance) {
    ...
    if (nHeight >= params.nSubsidyHalvingFirst && nHeight < params.nSubsidyHalvingSecond) {
        if (fShorterBlockDistance) {
    		...
    		FundPayment dataMiningFundPayment = params.nProjectExpensesFundPayment;
            dataMiningFundPayment.FillFundPayment(coinbaseTx, nHeight, nAmount,pblock->txoutProjectExpenses);
        }
        else {
            // Stage 2
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("RealichainMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(chainparams, std::shared_ptr<const CBlock>(new CBlock(*pblock)), true, NULL))
        return error("RealichainMiner: ProcessNewBlock, block not accepted");

    return true;
}

void static RealichainMiner(const CChainParams &chainparams) {
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("realichain-miner");
    ...
    try {
        ...
        if (!coinbaseScript || coinbaseScript->reserveScript.empty()) {
            LogPrintf("RealichainMiner stop here coinbaseScript=%s, coinbaseScript->reserveScript.empty()=%s\n", coinbaseScript, coinbaseScript->reserveScript.empty());
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in RealichainMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running RealichainMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        CheckWork(pblock, *pwallet, reservekey);
        ...
        LogPrintf("RealichainMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("RealichainMiner runtime error: %s\n", e.what());
        return;
    }
}
...
void GenerateBitcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RealichainMiner, boost::cref(chainparams)));
}
