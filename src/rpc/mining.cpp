...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        if(Params().NetworkIDString() == "main") 
            throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Realichain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Realichain is downloading blocks...");

    if (Params().GetConsensus().IsMain() && !masternodeSync.IsSynced())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Realichain Core is syncing with network...");
...
//    const bool fPreSegWit = (THRESHOLD_ACTIVE != VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT, versionbitscache));
	...
	FundPayment dataMiningFundPayment = Params().GetConsensus().nProjectExpensesFundPayment;
	if(pblock->txoutProjectExpenses != CTxOut()) {
		CTxDestination address;
		ExtractDestination(pblock->txoutProjectExpenses.scriptPubKey, address);
		CBitcoinAddress address2(address);
		dataMiningtObj.push_back(Pair("payee", address2.ToString().c_str()));
		dataMiningtObj.push_back(Pair("script", HexStr(pblock->txoutProjectExpenses.scriptPubKey.begin(), pblock->txoutProjectExpenses.scriptPubKey.end())));
		dataMiningtObj.push_back(Pair("amount", pblock->txoutProjectExpenses.nValue));
	}
