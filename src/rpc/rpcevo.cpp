...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The Realichain address to send the collateral to.\n"
        },
        {"collateralAmount",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The Realichain address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress",
            "%d. \"payoutAddress\"            (string, required) The Realichain address to use for znode reward payments.\n"
        },
        {"proTxHash",
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > paramIdx + 6) {
        ...
        if (!fundAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Realichain address: ") + request.params[paramIdx + 6].get_str());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() >= 6) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Realichain address: ") + request.params[5].get_str());
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Realichain address: ") + request.params[4].get_str());
        FundSpecialTx(pwallet, tx, ptx, feeSourceAddress.Get());
...
void protx_quick_setup_help(CWallet* const pwallet)
{
    throw std::runtime_error(
			...
			"  \"realichain.conf\" :       (string) The content of realichain.conf to be used in vps node.\n"
            "}\n"
			"\nExamples:\n"
...
UniValue createConfigFile(std::string blsPrivateKey, std::string ip, std::string address) {
	...
	std::string fileName = GetDataDir().string() + "/" + address + "_" + split_str(ip,":") + "_realichain.conf";
...
UniValue protx_quick_setup(const JSONRPCRequest& request) {
	...
	result.push_back(Pair("realichain.conf",config.get_str()));
