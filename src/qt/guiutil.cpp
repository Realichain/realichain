...
static QString realichainTheme = "realichainTheme";
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Realichain address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no realichain: URI
    if(!uri.isValid() || uri.scheme() != QString("realichain"))
        return false;
...
{
    // Convert realichain:// to realichain:
    //
    //    Cannot handle this later, because realichain:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("realichain://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "realichain:");
    }
...
{
    QString ret = QString("realichain:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Realichain.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Realichain (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Realichain (%s).lnk", chain);
}
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "realichain.desktop";
    return GetAutostartDir() / strprintf("realichain-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a realichain.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Realichain\n";
        else
            optionFile << strprintf("Name=Realichain (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Realichain app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Realichain app to startup item list
...
void loadTheme()
{
    ...
    QString fileName = stylesheetDirectory + "/" + realichainTheme;
