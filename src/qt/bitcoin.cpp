...
{
    return QCoreApplication::translate("realichain-core", psz).toStdString();
}
...
public:
    ...
    // migrate settings to realichain. Returns true if there was migration
    ...
    // migrate directories to realichain if needed
    void migrateToRealichain();
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        // realichain: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Realichain can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
void BitcoinApplication::migrateToRealichain()
{
    migrateSettings("Zcoin", "Realichain", "Zcoin-Qt", "Realichain-Qt");
    migrateSettings("Zcoin", "Realichain", "Zcoin-Qt-testnet", "Realichain-Qt-testnet");
    ...
    boost::filesystem::path realichainDefaultDataDir = GetDefaultDataDirForCoinName("realichain");

    if (dataDir != zcoinDefaultDataDir)
        return;

    boost::filesystem::path dontMigrateFilePath = dataDir / ".dontmigratetorealichain";
    if (boost::filesystem::exists(dontMigrateFilePath) && !GetBoolArg("-migratetorealichain", false))
        return;
    ...
    QTextStream(&messageText) <<
        "Migrate directory structure from zcoin to realichain? "
        "Directory " << GUIUtil::boostPathToQString(zcoinDefaultDataDir) <<
          " will be renamed to " << GUIUtil::boostPathToQString(realichainDefaultDataDir) <<
          " and file zcoin.conf in it will be renamed to realichain.conf";
    ...
    if (messageBox.exec() == QMessageBox::Yes) {
        if (RenameDirectoriesFromZcoinToRealichain()) {
            // Update path in settings if not set to non-default value
            setDataDirInSettings("Realichain", "Realichain-Qt", GUIUtil::boostPathToQString(realichainDefaultDataDir));
            setDataDirInSettings("Realichain", "Realichain-Qt-testnet", GUIUtil::boostPathToQString(realichainDefaultDataDir));
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    app.migrateToRealichain();

    /// 6. Determine availability of data directory and parse realichain.conf
    ...
    // realichain: links repeatedly have their payment requests routed to this process:
