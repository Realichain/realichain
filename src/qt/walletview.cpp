...
{
    // Create Realichain transactions list
    realichainTransactionList = new TransactionView(platformStyle);

    connect(realichainTransactionList, &TransactionView::message, this, &WalletView::message);

    // Create export panel for Realichain transactions
    ...
    connect(exportButton, &QPushButton::clicked, realichainTransactionList, &TransactionView::exportClicked);
    ...
    auto realichainLayout = new QVBoxLayout();
    realichainLayout->addWidget(realichainTransactionList);
    realichainLayout->addLayout(exportLayout);
    // TODO: fix this
    connect(overviewPage, &OverviewPage::transactionClicked, realichainTransactionList, qOverload<const QModelIndex&>(&TransactionView::focusTransaction));
    connect(overviewPage, &OverviewPage::outOfSyncWarningClicked, this, &WalletView::requestedSyncWarningInfo);

    realichainTransactionsView = new QWidget();
    realichainTransactionsView->setLayout(realichainLayout);
    ...
    if (isElysiumEnabled()) {
        ...
        transactionTabs->addTab(realichainTransactionsView, tr("Realichain"));
...
#endif
        pageLayout->addWidget(realichainTransactionsView);
...
{
    sendRealichainView = new SendCoinsDialog(platformStyle);

    connect(sendRealichainView, &SendCoinsDialog::message, this, &WalletView::message);
    ...
    if (isElysiumEnabled()) {
        ...
        sendCoinsTabs->addTab(sendRealichainView, tr("Realichain"));
...
#endif
        pageLayout->addWidget(sendRealichainView);
...
void WalletView::setClientModel(ClientModel *_clientModel)
{
    ...
    sendRealichainView->setClientModel(clientModel);
...
void WalletView::setWalletModel(WalletModel *_walletModel)
{
    ...
    realichainTransactionList->setModel(_walletModel);
    ...
    sendRealichainView->setModel(_walletModel);
...
void WalletView::focusBitcoinHistoryTab(const QModelIndex &idx)
{
    ...
    realichainTransactionList->focusTransaction(idx);
}
...
void WalletView::gotoSendCoinsPage(QString addr)
{
    ...
    if (!addr.isEmpty()){
        sendRealichainView->setAddress(addr);
    }
}
...
bool WalletView::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    ...
    return sendRealichainView->handlePaymentRequest(recipient);
}
